...
FL_OBJECT *FLpanel[17][17], *FLhidden, *FLexit,*FLselect1_grp, *FLselect2_grp, *FLinp_dummy,
          ...
          *FL_actIni, *FL_actInif, *FL_actAdj, *FLmod_grp,
...
int  g_cc_tosock = 0, g_cc_start = 0 ;
...
void *TakePict_thA(void *threadid) ;
...
int gen_logfile()
{
    ...
    AMCtime(  utime ) ;
    sprintf(fname,"AMC_%06d_%06d.log",utime[1],utime[0]);

    f_log = fopen(fname , "w");
    setlinebuf( f_log ) ; //flush buffer at each end of line

    f_err = fopen("AMC_err.log", "a");  //append to standard file
  ...
  for ( i=0; i<17; i++ )             
     for ( j=0; j<17; j++)
        if ( panel[i][j].pan_stat == STAT_ACT ) {
           ...
           if (imod == 2) {
              sprintf(a_sb_name,"/home/amc/sbig2/ROQ_%06d_%06d_Z%+03d_P%+02d%+02d_A%+05d_B%+05d",
                   utime[1], utime[0], jz-100, ii, jj,
                   panel[i][j].pc_mot[0], panel[i][j].pc_mot[1]);
           } else {
              sprintf(a_sb_name,"/home/amc/sbig2/POL_%06d_%06d_Z%+03d_P%+02d%+02d_A%+05d_B%+05d",
                   utime[1], utime[0], jz-100, ii, jj,
                   panel[i][j].pc_mot[0], panel[i][j].pc_mot[1]);
           }
           if ( g_sbig_err >0 ) {
...
//if ( g_sbigon !=1 && g_sbigon !=-999) { // for tests only
  ...
  sprintf(a_sb_name,"/home/amc/sbig2/STR_%06d_%06d_Z%+03d_PGR_%02d_DF_%04d.txt",
  ...
  sprintf(a_sb_name,"/home/amc/sbig2/STR_%06d_%06d_Z%+03d_PGR_%02d_DF_%04d",
          utime[1], utime[0], jz-100, p, idefoc) ;
     ...
     for ( j=0; j<17; j++)
        if ( panel[i][j].pan_stat == STAT_ACT ) {
           ...
           for ( k=knull; k<kmax; k++ ) {
              ...
              AMCtime( utime ) ;
              if (imod == 0 ) {
                 sprintf(a_sb_name,"/home/amc/sbig2/CAL_%06d_%06d_Z%+03d_P%+02d%+02d_A%+05d_B%+05d",
                      utime[1], utime[0], jz-100, i-8, j-8, calstep[0][k], calstep[1][k] );
              } else {
                 sprintf(a_sb_name,"/home/amc/sbig2/LAS_%06d_%06d_Z%+03d_P%+02d%+02d_A%+05d_B%+05d",
                      utime[1], utime[0], jz-100, i-8, j-8, calstep[0][k], calstep[1][k] );
              }
  ...
  for ( i=0; i<17; i++ )
     for ( j=0; j<17; j++)
        if ( panel[i][j].pan_stat == STAT_ACT ) {
           ...
           AMCtime( utime ) ;
           sprintf(a_sb_name,"/home/amc/sbig2/LAS_%06d_%06d_Z%+03d_P%+02d%+02d_A%+05d_B%+05d_Z%+03d",
                      utime[1], utime[0], jz-100, ii, jj,
...
if ( obj != NULL && ( n==PWR_SBIG || n==PWR_DIST) ) {
   ...
   return ;
                                       ...
                                       sprintf(lstr,"power button %d pressed",n);
                                       put_logfile( LOG_DB1 , 0, lstr) ;
  ...
  if ( n == 900 ) {                                                //default
     ...
     if ( n == 990 ) {
        ...
        if ( n<7 ) power[n+8].request = ibut ;                     //set both channels for chains
     }
  }
}
...
//-----
   ...
   if ( g_initgui == 0 ) {   //have to do some initializations ....
      ...
      mode_cb( NULL, MODE_EXPT ) ;       //set default mode
...
void save_cb_old(FL_OBJECT *ob, long n)
{
   const char *s;
   ...
   if ( n>0 ) {
      ...
      AMCtime(  utime ) ;
      sprintf(fname,"/home/amc/sbig2/AMC_%06d_%06d_F%d_X%04d_Z%+03d_%s.fits",
      utime[1],utime[0],g_pict_filter,g_pict_expos,(int)pct_zenit,fnam0);
  }  else
      strncpy( fname, a_sb_name, 80) ;

                                        sprintf(lstr,"save %s",fname) ;
                                        put_logfile( LOG_SBG ,0, lstr) ;
                                        sprintf(lstr,"save %s",fnam0) ;
                                        put_logfile( LOG_SBG ,0, lstr) ;
...
void save_cb(FL_OBJECT *ob, long n)
{
  ...
  if ( n>0 ) {
     ...
     AMCtime(  utime ) ;
     snprintf(fname,LOGLEN,"/home/amc/sbig2/AMC_%06d_%06d_F%d_X%04d_Z%+03d_%s.fits",
         utime[1],utime[0],g_pict_filter,g_pict_expos,(int)pct_zenit,fnam0);
     snprintf(fnamt,LOGLEN,"/home/amc/sbig2/AMC_%06d_%06d_F%d_X%04d_Z%+03d_%s.motor",
         utime[1],utime[0],g_pict_filter,g_pict_expos,(int)pct_zenit,fnam0);
...
//                                     put_logfile( LOG_DB1 , 0, lstr) ;
  ...
  if ( n == -1 ) {      // set temperature
     ...
     m = ( n % 10 ) ;
     if ( m == 1 )     g_sb_shutt = SC_OPEN_SHUTTER ;
     else if ( m == 2) g_sb_shutt = SC_CLOSE_SHUTTER ;
     else if ( m == 3) g_sb_shutt = SC_LEAVE_SHUTTER ;
     else              g_sb_mode  = S_NULL_TH ;
...
//  switch on power if needed ....
    ...
    while ( power[PWR_SBIG].actual < 1 && pwrtry++ < 20 ) sleep(1) ;
...
void *TakePict_th(void *threadid)
{
   ...
   g_sbig_dscr  = 1./time ;
   sprintf(lstr,"SBIG Pict %d %f",g_sb_mode,time);
   put_logfile( LOG_SBG , 0, lstr) ;
...
int read_from_CC(int filedes)
{
   ...
   long   ccst0,ccyr,ccmon,ccday,cchour,ccmin,ccsec,ccmsec;
   long   ccst1[31];
   float  ccM1zd,ccM1az,ccRQzd,ccRQaz,ccT,ccP,ccV,ccH,ccUPS,ccGPS;
   char   ccsrc[LOGLEN],ccsched[100],xName[LOGLEN];
   long   cccat;
   long   ccst2[16];
   ...
   for ( k=0; k<nbytes; k++) {
      if (buffer[k] != '\n'  &&  buffer[k] != '\0' ) {
         ...
         if ( strncmp(g_ccbuf,"CC-",3) == 0 ) { // must be a report ...
            ...
            n=sscanf(g_ccbuf,"CC-REPORT %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %f %f %f %f %f %f %f %f %f %f %s %s %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %f %f",
              &ccst0, &ccyr, &ccmon, &ccday, &cchour, &ccmin, &ccsec, &ccmsec,
              &ccst1[ 0],&ccst1[ 1],&ccst1[ 2],&ccst1[ 3],&ccst1[ 4],&ccst1[ 5],&ccst1[ 6],&ccst1[ 7],&ccst1[ 8],&ccst1[ 9],
              &ccst1[10],&ccst1[11],&ccst1[12],&ccst1[13],&ccst1[14],&ccst1[15],&ccst1[16],&ccst1[17],&ccst1[18],&ccst1[19],
              &ccst1[20],&ccst1[21],&ccst1[22],&ccst1[23],&ccst1[24],&ccst1[25],&ccst1[26],&ccst1[27],&ccst1[28],&ccst1[29],
              &ccst1[30],
              &ccM1zd, &ccM1az, &ccRQzd, &ccRQaz, &ccT, &ccP, &ccV, &ccH, &ccUPS, &ccGPS,
              &ccsched, &ccsrc, &cccat,
              &ccst2[ 0],&ccst2[ 1],&ccst2[ 2],&ccst2[ 3],&ccst2[ 4],&ccst2[ 5],&ccst2[ 6],&ccst2[ 7],&ccst2[ 8],&ccst2[ 9],
              &ccst2[10],&ccst2[11],&ccst2[12],&ccst2[13],&ccst2[14],&ccst2[15],
              &ccM2zd, &ccM2az);
            ...
            if (        strncmp(g_ccbuf,"BREAK",5)==0 ) {
               ...
               snprintf(g_cc_name,LOGLEN,"/home/amc/sbig2/TPT_%06d_%06d_F%d_X%04d_Z%+03d_%s",
                   utime[1],utime[0],g_pict_filter,g_pict_expos,global_azimut,xName);
            } else {
...
//  int sock, errv;
  ...
  while ( g_ccsnd >=0 ) { 
    ...
    sprintf(report,"AMC-REPORT M2 %02d %04d %02d %02d %02d %02d %02d %03d %02d %04d %02d %02d %02d %02d %02d %03d PANELS %03d %03d %03d %03d ADJUST %05d %05d %05d %05d AUTOAD %05d %05d %05d %05d OVER\n",
...
{ 
  ...
  sprintf(tstr,"MAGIC-II AMC Beta V4.10a");
