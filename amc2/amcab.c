...
int PWRPROBLEM = 1;  //set to 0 when all ok, else it suppresses the error messages
...
//char sbig_path[256] = "/home/operator/sbig2_local";
char sbig_path[256] = "/home/operator/sbig2";
...
goodpan:   //make sure laser is off !!!!  and readout thread terminated
        ...
        g_sbig_err = 0;
        if (numbad > 1 || global_break > 0)
          goto endlasi;   //2 consecutive SBIG problems==>stop
      }
      ...
      for (i = 0; i < 17; i++)
        for (j = 0; j < 17; j++) {
          ...
          if (panel[i][j].pan_stat == STAT_TDO) {
            ...
            ix = panel[i][j].lut[0][jz][0];
            iy = panel[i][j].lut[0][jz][1];
            if (ix < 0) panel[i][j].pc_moveto[0] = ix + 2500;
            else panel[i][j].pc_moveto[0] = ix - 2500;
            if (iy < 0) panel[i][j].pc_moveto[1] = iy + 2500;
            else panel[i][j].pc_moveto[1] = iy - 2500;
          }
        }
    }
...
//                 and make sure pc_moveto is even ....
              ...
              if (abs(panel[i][j].move_by[0]) > 3  || abs(panel[i][j].move_by[1]) > 3
                ...
                printf(" =============== (%2d,%2d) moveby %6d, %6d |  moveto %6d, %6d | act %6d %6d\n", i - 8, j - 8,
                       panel[i][j].move_by[0], panel[i][j].move_by[1],
                       panel[i][j].pc_moveto[0], panel[i][j].pc_moveto[1],
                       panel[i][j].act_mot[0], panel[i][j].act_mot[1]);
      ...
      for (i = 0; i < 17; i++)
        for (j = 0; j < 17; j++)
          if (panel[i][j].pan_stat == STAT_TDO) {
            ...
            panel[i][j].act_mot[0] = panel[i][j].act_mot[1] = -8788;
            panel[i][j].pc_mot[0]  = panel[i][j].pc_mot[1]  = -8688;
          }
          else if (panel[i][j].pan_stat == STAT_NIN) nudf0++;
          else if (panel[i][j].pan_stat == STAT_ERR) nerr0++;
          else if (panel[i][j].pan_stat == STAT_ER1) nerr0++;
...
//                    do we have a reasonable query even if not initialized ???
                ...
                if (panel[i][j].pc_moveto[0] == -8888 && abs(panel[i][j].act_mot[0] - panel[i][j].pc_mot[0]) <= 1) {
//     sprintf(lstr,"reasonable query... (%d,%d):%d, %d, %d",i-8,j-8,panel[i][j].port[0][0],panel[i][j].act_mot[0],panel[i][j].pc_mot[0]);
//     put_logfile(LOG_ERR, 0, lstr);
                  panel[i][j].pc_moveto[0] = panel[i][j].act_mot[0];
                }
                if (panel[i][j].pc_moveto[1] == -8888 && abs(panel[i][j].act_mot[1] - panel[i][j].pc_mot[1]) <= 1) {
//     sprintf(lstr,"reasonable query... (%d,%d):%d, %d, %d",i-8,j-8,panel[i][j].port[0][0],panel[i][j].act_mot[1],panel[i][j].pc_mot[1]);
//     put_logfile(LOG_ERR, 0, lstr);
...
//                          abs(panel[i][j].act_mot[1])<6000) {
//
//     sprintf(lstr,"maybe fast init ... (%d,%d), %d, %d",i-8,j-8,panel[i][j].act_mot[1],panel[i][j].pc_mot[1]);
//     put_logfile(LOG_ERR, 0, lstr);
          ...
          for (j = 0; j < 17; j++)
            if (panel[i][j].pan_stat > STAT_BAD)
              if (abs(panel[i][j].act_mot[0]) > 6000 ||
                ...
                abs(panel[i][j].act_mot[0] - panel[i][j].pc_moveto[0]) > 4  ||
                abs(panel[i][j].act_mot[1] - panel[i][j].pc_moveto[1]) > 4)
                panel[i][j].mot_stat = panel[i][j].pan_stat = STAT_ERR;
              else
                panel[i][j].mot_stat = panel[i][j].pan_stat = STAT_OK_;
      }
...
void push_pwr(FL_OBJECT *obj, long m)
{
  ...
  if (n == 900) {                                                //default
    ...
    for (j = 0; j < 32; j++) {
      power[j].request = abs(power[j].deflt);
    }
    g_pwr_req++;
  }
...
void gen_panel(int jx, int jy, int dxx, int dyy)
{
  ...
  FL_durat =
    obj = fl_add_box(FL_FLAT_BOX, jx + 7 * dxx, jy + 8.5 * dyy, 2 * dxx, dyy / 2., "INITIALISING");
  fl_set_object_lsize(obj, FL_MEDIUM_SIZE);
  fl_set_object_lstyle(obj, FL_FIXEDBOLD_STYLE);
  ...
  dx = dxx * 0.70;                       // chain-group select
  dy = dyy * 0.50;
  ix = jx;
  for (ip = 1; ip < 5; ip++) {
    ...
    if ((g_err + g_nin) > 0) {
      ...
      if (ret == 1) {
        ...
        */
        ...
        cmd = GUIcmd_CNTR;
        fl_set_button(obj, 1);   //set corresponding action-button
        action_cb(obj,  cmd);   //this is done in own thread; it will set g_initgui=+4 when done
        fl_set_button(obj, 0);   //deselect action-button
        // Adjust
        obj = FL_actAdj;
        cmd = GUIcmd_ADJS;
    ...
    if (g_cycimg > 0) {
      ...
      while (cf < g_cycimg) {
        ...
        if (g_sbig_th <= 0 && g_sbig_th != -S_FILT_TH) {
          ...
          else g_filter = 4; // start with green, M1 starts with blue
          g_pict_filt = g_filter;
...
//
//uncomment to activate modified AMC-Report including the ifID used ...
//  sprintf(report, "AMC-REPORT M2 %02d %04d %02d %02d %02d %02d %02d %03d %02d %04d %02d %02d %02d %02d %02d %03d PANELS %03d %03d %03d %03d ADJUST %05d %05d %05d %05d AUTOAD %05d %05d %05d %05d CCD %1d %04d %1d %03d ifID %05d OVER\n",
//          l_AMCstat,  yy,  mon,  dd,  hh,  mm,  ss,  ms,
//          g_COMstat, g_yy, g_mon, g_dd, g_hh, g_mm, g_ss, g_ms,
//          g_all, g_err + g_com, g_mov, g_nin,
//          g_zd0, g_az0, g_fc0, g_tim0,
...
int main(int argc, char *argv[])
{
  ...
  sprintf(tstr, "MAGIC-II AMC V4.50");
