...
#include "magpos2.inc"
#include "amcmotor_m2.inc"               //most of it can be removed ?!!!!
...
#include "HAmaindef_m2.h"
#include "hamain_m2.inc"

    for (ki=0; ki<16; ki++) fdreal[ki]    = fd[ki] ;
    for (ki=0; ki<4 ; ki++) fd232real[ki] = fd232[ki] ;
...
int AMC_portswap(int list[32])
{
  int k ;

//  for (k=0; k<16; k++) fd[k]    = fdreal[k] ;
//  for (k=0; k<4 ; k++) fd232[k] = fd232real[k] ;

  for (k=0; k<8; k++) 
    if (list[k] < 100 ) fd[k] = fdreal[k] ;
    else                fd[k] = fd232real[k] ;

  for (k=8; k<12; k++)
    if (list[k] < 100 ) fd232[k] = fdreal[k] ;
    else                fd232[k] = fd232real[k] ;
