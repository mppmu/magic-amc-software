...
// for the following items: index 0=x-motor, 1=y, 2=laser
   ...
   int curr[3], Vsup[3];          //current    , Supply Voltage
   int V2[3],   Vlog[3];          //V2         , Logic Voltage
   //
...
status[][0]: bit 0: verges error, execution of command done/tried
             ...
             bit 7: bad driver address

status[][1]: bit 0: x motor: moving
             bit 1: y motor: moving
             bit 2: x motor: direction up
             bit 3: y motor: direction up
             bit 4: x motor: in endswitch
             bit 5: y motor: in endswitch
             bit 6: laser on
             bit 7: no response from driver card/ serious hardware problem
...
// unsigned int watch0[3];        //watchdog value at first call  NO WATCHDOG for AMC I
...
// unsigned char watsrc[3];       //watchdog source
   ...
   int pan_centred;               // 1: panel got just centred, i.e. x/y = 0/0 is legal
                                  //-1: panel was centred,  x/y = 0/0 reached by move command
                                  // 0: panel NOT centred,  x/y position is arbitrary
   ...
   int samebox[3];                //().pnmbr of the other 3 panels contacted by the same box
}                                 //
AMCpanel ;                      
...
#define  STAT_BCM    -1           // box communication problems
#define  STAT_NIN     0           // panel not initialised
